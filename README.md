...
Все функции в Python являются объектами класса `function`. Соответтсвенно, с ними возможно делать всё то, что и с обычными классами - обращаться к методам экземпляра, присваивать переменным, пережавать в другие функции и т.д..
```python
...
print(list(map(factorial, filter(lambda i: i % 2, range(10)))))  # С помощью спискового включения
print([factorial(i) for i in range(10) if i % 2])                # С помощь. `map` и `filter`
```
