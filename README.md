...
## Объект как функция
Для того, чтобы предоставить объекту быть вызванным, как функция, через `()` необходимо реализовать в нём метод `__call__.` В итоге получится "функция", которая запоминает внутреннее состояние между вызовами.
```python
import random
